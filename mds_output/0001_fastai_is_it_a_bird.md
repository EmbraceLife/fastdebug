...
keyword match is  <mark style="background-color: #ffff00">1.0</mark> , found a line: in  <mark style="background-color: #FFFF00">_rm_self.py</mark> 
        ...
        sigd = dict(sig.parameters)===========================================================(1) # [93;1mhow to access parameters from a signature[0m; [91;1mhow is parameters stored in sig[0m; [36;1mhow to turn parameters into a dict;[0m; 
...
the entire source code in  <mark style="background-color: #FFFF00">_rm_self.py</mark> 
    ...
    def _rm_self(sig):========================================================================(0) # [35;1mremove parameter self from a signature which has self;[0m; 
        ...
        return sig.replace(parameters=sigd.values())==========================================(3) # [34;1mhow to update a sig using a updated dict of sig's parameters[0m; 
                                                                                                                                                            (4)
...
keyword match is  <mark style="background-color: #ffff00">1.0</mark> , found a line: in  <mark style="background-color: #FFFF00">delegates.py</mark> 
            ...
            sigd = dict(sig.parameters)=======================================================(12) # [36;1mHow to access parameters of a signature?[0m; [92;1mHow to turn parameters into a dict?[0m; 
...
the entire source code in  <mark style="background-color: #FFFF00">delegates.py</mark> 
    ...
    def delegates(to:FunctionType=None, # Delegatee===========================================(0) # [91;1mhow to make delegates(to) to have to as FunctionType and default as None[0m; 
        ...
        def _f(f):============================================================================(5)       
            if to is None: to_f,from_f = f.__base__.__init__,f.__init__=======================(6) # [92;1mhow to write 2 ifs and elses in 2 lines[0m; 
            else:          to_f,from_f = to.__init__ if isinstance(to,type) else to,f=========(7) # [92;1mhow to assign a,b together with if and else[0m; 
            from_f = getattr(from_f,'__func__',from_f)========================================(8) # [91;1mIs classmethod callable[0m; [36;1mdoes classmethod has __func__[0m; [93;1mcan we do inspect.signature(clsmethod)[0m; [35;1mhow to use getattr(obj, attr, default)[0m; 
            to_f = getattr(to_f,'__func__',to_f)==============================================(9)       
            if hasattr(from_f,'__delwrap__'): return f========================================(10) # [35;1mif B has __delwrap__, can we do delegates(A)(B) again?[0m; [93;1mhasattr(obj, '__delwrap__')[0m; 
            sig = inspect.signature(from_f)===================================================(11) # [92;1mhow to get signature obj of B[0m; [34;1mwhat does a signature look like[0m; [34;1mwhat is the type[0m; 
            sigd = dict(sig.parameters)=======================================================(12) # [36;1mHow to access parameters of a signature?[0m; [92;1mHow to turn parameters into a dict?[0m; 
            k = sigd.pop('kwargs')============================================================(13) # [35;1mHow to remove an item from a dict?[0m; [35;1mHow to get the removed item from a dict?[0m; [36;1mHow to add the removed item back to the dict?[0m; [92;1mwhen writing expressions, as they share environment, so they may affect the following code[0m; 
            s2 = {k:v.replace(kind=inspect.Parameter.KEYWORD_ONLY) for k,v in inspect.signature(to_f).parameters.items() # [92;1mHow to access a signature's parameters as a dict?[0m; [36;1mHow to replace the kind of a parameter with a different kind?[0m; [34;1mhow to check whether a parameter has a default value?[0m; [92;1mHow to check whether a string is in a dict and a list?[0m; [34;1mhow dict.items() and dict.values() differ[0m;  (14)
                  if v.default != inspect.Parameter.empty and k not in sigd and k not in but}=(15)      
            anno = {k:v for k,v in getattr(to_f, "__annotations__", {}).items() if k not in sigd and k not in but} # [35;1mHow to get A's __annotations__?[0m; [34;1mHow to access it as a dict?[0m; [35;1mHow to select annotations of the right params with names?[0m; [36;1mHow to put them into a dict?[0m; [93;1mHow to do it all in a single line[0m;  (16)
            sigd.update(s2)===================================================================(17) # [93;1mHow to add the selected params from A's signature to B's signature[0m; [34;1mHow to add items into a dict;[0m; 
            if keep: sigd['kwargs'] = k=======================================================(18) # [35;1mHow to add a new item into a dict;[0m; 
            else: from_f.__delwrap__ = to_f===================================================(19) # [35;1mHow to create a new attr for a function or obj;[0m; 
            from_f.__signature__ = sig.replace(parameters=sigd.values())======================(20) # [34;1mHow to update a signature with a new set of parameters;[0m; 
            if hasattr(from_f, '__annotations__'): from_f.__annotations__.update(anno)========(21) # [91;1mHow to check whether a func has __annotations__[0m; [36;1mHow add selected params' annotations from A to B's annotations;[0m; 
            return f==========================================================================(22)      
        return _f=============================================================================(23)      
                                                                                                                                                            (24)
...
```
     ...
     [91;1mhow does a metaclass create a class instance[0m; [35;1mwhat does super().__new__() do here;[0m; 
     [91;1mhow to remove self from a signature[0m; [91;1mhow to check whether a class' __init__ is inherited from object or not;[0m;  (4)
    ...
    multiply instead of add 
     ...
     [35;1mhow to run superclass' __init__ function[0m; 
     ...
     [93;1mhow to access parameters from a signature[0m; [91;1mhow is parameters stored in sig[0m; [36;1mhow to turn parameters into a dict;[0m; 
     [91;1mhow to remove the self parameter from the dict of sig;[0m; 
     [34;1mhow to update a sig using a updated dict of sig's parameters[0m; 
     pprint and inspect is loaded from fastdebug
     Delegatee===========================================(0)  Keep `kwargs` in decorated function?==========================(1)       
     Exclude these parameters from signature===================(2)  [92;1mhow to write 2 ifs and elses in 2 lines[0m; 
     [92;1mhow to assign a,b together with if and else[0m; 
     [91;1mIs classmethod callable[0m; [36;1mdoes classmethod has __func__[0m; [93;1mcan we do inspect.signature(clsmethod)[0m; [35;1mhow to use getattr(obj, attr, default)[0m; 
     [35;1mif B has __delwrap__, can we do delegates(A)(B) again?[0m; [93;1mhasattr(obj, '__delwrap__')[0m; 
     [92;1mhow to get signature obj of B[0m; [34;1mwhat does a signature look like[0m; [34;1mwhat is the type[0m; 
     [36;1mHow to access parameters of a signature?[0m; [92;1mHow to turn parameters into a dict?[0m; 
     [35;1mHow to remove an item from a dict?[0m; [35;1mHow to get the removed item from a dict?[0m; [36;1mHow to add the removed item back to the dict?[0m; [92;1mwhen writing expressions, as they share environment, so they may affect the following code[0m; 
     [92;1mHow to access a signature's parameters as a dict?[0m; [36;1mHow to replace the kind of a parameter with a different kind?[0m; [34;1mhow to check whether a parameter has a default value?[0m; [92;1mHow to check whether a string is in a dict and a list?[0m; [34;1mhow dict.items() and dict.values() differ[0m;  (14)
     [35;1mHow to get A's __annotations__?[0m; [34;1mHow to access it as a dict?[0m; [35;1mHow to select annotations of the right params with names?[0m; [36;1mHow to put them into a dict?[0m; [93;1mHow to do it all in a single line[0m;  (16)
     [93;1mHow to add the selected params from A's signature to B's signature[0m; [34;1mHow to add items into a dict;[0m; 
     [35;1mHow to add a new item into a dict;[0m; 
     [35;1mHow to create a new attr for a function or obj;[0m; 
     [34;1mHow to update a signature with a new set of parameters;[0m; 
     [91;1mHow to check whether a func has __annotations__[0m; [36;1mHow add selected params' annotations from A to B's annotations;[0m; 
     set with __pre_init__
     set with __init__
     set with __post_init__
     [92;1mPrePostInitMeta inherit __new__ and __init__ from FixSigMeta as a metaclass (a different type)[0m; [36;1mnot from type, nor from object[0m; [93;1mPrePostInitMeta is itself a metaclass, which is used to create class instance not object instance[0m; [92;1mPrePostInitMeta writes its own __call__ which regulates how its class instance create and initialize object instance[0m; 
     [93;1mhow to create an object instance with a cls[0m; [36;1mhow to check the type of an object is cls[0m; [35;1mhow to run a function without knowing its params;[0m; 
     [36;1mhow to run __init__ without knowing its params[0m; 
     allows you to add method b upon instantiation
     don't forget to include **kwargs in __init__
     the attempt to add a is ignored and uses the original method instead.
...
![png](0001_fastai_is_it_a_bird_files/0001_fastai_is_it_a_bird_49_0.png)
...
![png](0001_fastai_is_it_a_bird_files/0001_fastai_is_it_a_bird_50_1.png)
